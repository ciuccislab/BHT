...
def NMLL_fct(theta, Z, A, L, N_freqs, N_taus):
    
    sigma_n, sigma_beta, sigma_lambda = theta

    # keep as above
    W = 1/(sigma_beta**2)*np.eye(N_taus+1) + 1/(sigma_lambda**2)*L.T@L
...
def HT_single_est(theta_0, Z_exp, A, A_H, L, N_freqs, N_taus):
    ...
    res = minimize(NMLL_fct, theta_0, args=(Z_exp, A, L, N_freqs, N_taus), callback=print_results, options={'gtol': 1E-8, 'disp': True})
